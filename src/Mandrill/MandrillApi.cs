...
namespace Mandrill
{
    ...
    public partial class MandrillApi
    {
        ...
        /// <summary>
        /// PostSync (uses synchronous function right now because PostAsync has a bug)
        /// </summary>
        /// <param name="path"></param>
        /// <param name="data"></param>
        /// <returns></returns>
        public IRestResponse PostSync(string path, dynamic data)
        {
            var request = new RestRequest(path, Method.POST);
            request.RequestFormat = DataFormat.Json;
            ...
            {
                var error = JSON.Parse<ErrorResponse>(response.Content);
                var ex = new MandrillException(error, response.ErrorMessage);
